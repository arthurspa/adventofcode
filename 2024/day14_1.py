...
def solve(input: TextIOWrapper):
    ...
    mid_rows = rows // 2
    mid_cols = cols // 2
    top_left = (0, mid_rows, 0, mid_cols)
    top_right = (0, mid_rows, mid_cols + 1, cols)
    botom_right = (mid_rows + 1, rows, mid_cols + 1, cols)
    botom_left = (mid_rows + 1, rows, 0, mid_cols)
    quadrants_list = (top_left, top_right, botom_right, botom_left)
    quadrants = dict()
    ...
    for line in input:
        ...
        for quadrant in quadrants_list:
            if robot.is_in_quadrant(quadrant):
                quadrants[quadrant] += 1

    safety_factor = (
        quadrants[top_left]
        * quadrants[top_right]
        * quadrants[botom_right]
        * quadrants[botom_left]
    )
